...
def clean_feature(feature_in: dict, to_places: int = 1) -> dict:
    ...
    feature_in["geometry"]["coordinates"] = round_coordinates(
        feature_in["geometry"]["coordinates"], to_places
    )
    return feature_in
...
def reindent_geojson(geojson: str) -> str:
    ...
    geojson = re.sub(r"\[\s+(-*\d+\.\d*),\s*(-*\d+\.\d*)\s*\]", r"[ \1, \2 ]", geojson)
...
def clean_geojson(file_path: str, key="fid") -> str:
    ...
    raw["features"] = sorted(raw["features"], key=lambda x: x["properties"].get(key))

    output = json.dumps(obj=raw, indent=1)
...
if __name__ == "__main__":
    dir_in = os.path.join(os.path.split(os.getcwd())[0], "maps")
