...
#--Cycle through, opening and resizing files----------------------------------
        for srcFile in srcFileList:
            if (tgtFileDict[srcFile] not in existingList) or overwrite:
                ...
                res = (pdb.gimp_image_width(imageIn),
                    pdb.gimp_image_height(imageIn))
                xScal, yScal = (float(resOut) / res[0],
                    float(resOut) / res[1])
                xScal, yScal = (int(min(xScal, yScal) * res[0]),
...
#--Reshape the image to a square if requested---------------------------------
                if square:
                    ...
                    pdb.gimp_image_resize(
                        imageIn, addTo[0] + xScal, addTo[1] + yScal,
                        addTo[0] / 2, addTo[1] / 2
                    )
                    pdb.gimp_layer_resize(
                        pdb.gimp_image_get_active_layer(imageIn),
                        addTo[0] + xScal, addTo[1] + yScal,
                        addTo[0] / 2, addTo[1] / 2
                    )
#--Save to disk & close-------------------------------------------------------
                ...
                pdb.file_jpeg_save(
                    imageIn, drawableOut, tgtFile, tgtFile, 0.8, 0, 1, 1,
                    "Automatically re-sized output.", 0, 1, 0, 0
                )
